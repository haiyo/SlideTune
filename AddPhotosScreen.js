...
const AddPhotosScreen = ({ route, navigation }) => {
    ...
    useEffect(() => {
        if (longPress) {
            const headerButton = selectedPhotos.length === 0 ? (
                ...
                <TouchableWithoutFeedback onPress={() => handleDeletePhoto()}>
    ...
    const handleDeletePhoto = () => {
        // Remove the selected photos from the `photos` state array
        const updatedPhotos = photos.filter((photo) => !selectedPhotos.includes(photo));
        setPhotos(updatedPhotos);

        // Update the AsyncStorage with the updated photos
        AsyncStorage.setItem(album.id, JSON.stringify(updatedPhotos))
        .then(() => {
            // Reset the selected photos and longPress state
            setSelectedPhotos([]);
            setLongPress(false);
        })
        .catch((error) => {
            console.log("Error deleting photos:", error);
            // Handle any error that occurred while deleting the photos
        });
    };
            ...
            if (response.didCancel) {
                ...
                if(response.assets[0].type == 'video/mp4') {
                    try {
                        ...
                        source = filePath; // Return the file path
                    }
                    catch (error) {
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.buttonsContainer}>
                ...
                <TouchableOpacity style={[styles.actionButton, styles.actionMButton]} onPress={handleSelectPhotos}>
                    <FontAwesomeIcon icon={ faPlus } style={ styles.plusIcon } size={ 30 }  />
                </TouchableOpacity>

                {photos.length > 0 &&
                    <TouchableOpacity style={styles.actionButton} onPress={() => navigation.navigate('SlideShowScreen', { albumId: album.id, photos: photos })}>
                        <FontAwesomeIcon icon={ faPlay } style={ styles.plusIcon } size={ 25 }  />
                    </TouchableOpacity>
                }

                {/*<TouchableOpacity style={styles.actionButton} onPress={() => handleAddMusic()}>
                    <FontAwesomeIcon icon={ faMusic } style={ styles.plusIcon } size={ 25 }  />
                </TouchableOpacity>*/}
            </View>
        </View>
    );
